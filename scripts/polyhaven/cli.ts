...
async function main() {
  ...
  if (command === 'plan') {
    ...
    if (manifestPath && categories.length) throw new Error('--manifest and --category are mutually exclusive');
    ...
    if (manifestPath) {
      ...
      selectedIds = new Set(Object.entries(snapshot.records)
        .filter(([, record]) => {
          ...
          if (record.environmentGroup && categories.includes(record.environmentGroup.toLowerCase())) return true;
          return categories.some(c => assetCategories.includes(c));
        })
        .map(([id]) => id));
      if (selectedIds.size === 0) throw new Error(`No discovered assets matched category: ${categories.join(', ')}`);
    }
