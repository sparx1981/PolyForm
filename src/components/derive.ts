...
export function derive(
  ...
  // An edge is excluded from this pass entirely — never bucketed, never
  // re-examined — if it belongs ONLY to faces marked `chamferLocked` and
  // it is not itself in `touched`.
  //
  // This exists for one reason: regionsFor's own plane-bucketing builds a
  // candidate plane from every pair of edges meeting at a vertex, which is
  // correct for a simple degree-3 corner (a box) but not for the degree-4+
  // corners a chamfer produces, where some pairings are two edges that
  // happen to touch a point without ever being meant as one face's
  // boundary. On a symmetric chamfered solid, enough of those coincide
  // into a bogus region that correctly-built facets get silently replaced
  // — confirmed, and NOT fixed by two different attempts at correcting the
  // bucketing heuristic itself (see chamfer.ts's own notes).
  //
  // Rather than keep guessing at that heuristic, this sidesteps it for
  // ALREADY-CORRECT, marked geometry: if nothing is trying to touch it,
  // it never enters regionsFor's input at all, so it cannot be
  // mis-bucketed. The moment something DOES touch one of its edges — the
  // user editing that specific geometry again — this exclusion no longer
  // applies to that edge, and the original limitation still exists for
  // that specific, narrower case. That is a deliberate, documented
  // trade-off, not an oversight: excluding a touched edge here would mean
  // a real edit never took effect at all.
  //
  // For every model that has never used chamfer, no face ever has this
  // marker, so `onlyLocked` is false for every edge and this is a
  // complete no-op — confirmed against the full existing suite.
  const all = new Set<EdgeId>();
  for (const [eid, e] of g.edges) {
    if (touched.has(eid)) {
      all.add(eid);
      continue;
    }
    let onlyLocked = e.uses.length > 0;
    for (const use of e.uses) {
      const loop = g.loops.get(use.loop);
      const face = loop && g.faces.get(loop.face);
      if (!face?.attributes.custom?.chamferLocked) {
        onlyLocked = false;
        break;
      }
    }
    if (onlyLocked) continue;
    all.add(eid);
  }
