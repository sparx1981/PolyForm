...
const DATA_MODELS: DataModel[] = [
  ...
  {
    name: 'RoofAssembly',
    path: 'Model.roofAssemblies[] / sdk.architecture',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier for the roof assembly.' },
      { name: 'roofType', type: 'RoofType', description: 'gable | hip | mansard | shed | flat | dutch-hip | gambrel | butterfly | saltbox | parapet' },
      { name: 'width', type: 'number', description: 'Base footprint width in metres.' },
      { name: 'depth', type: 'number', description: 'Base footprint depth/length in metres.' },
      { name: 'pitchAngleDeg', type: 'number', description: 'Roof slope pitch angle in degrees (5°–75°).' },
      { name: 'eaveOverhang', type: 'number', description: 'Horizontal projection of eaves past perimeter walls (m).' },
      { name: 'fasciaHeight', type: 'number', description: 'Vertical thickness of eaves fascia boards (m).' },
      { name: 'tileShape', type: 'TileShapeProfile', description: 'none | roman | mission | spanish | flat | slate | shingle | barrel | scallop | interlocking | standing-seam' },
      { name: 'tileSize', type: 'number', description: 'Scale factor for generated 3D tiles (0.5 – 2.0).' },
      { name: 'tileColor', type: 'string', description: 'Hex colour string for base tile geometry.' },
      { name: 'randomizeColor', type: 'boolean', description: 'Whether tile colours are subtly jittered across colorPalette.' },
      { name: 'colorPalette', type: 'string[]?', description: 'Array of hex colours used for variegated shingle/tile distribution.' },
      { name: 'position', type: '[number,number,number]', description: 'World XYZ placement origin.' },
    ],
  },
  {
    name: 'StairAssembly',
    path: 'Model.stairAssemblies[] / sdk.architecture',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier for the stair flight.' },
      { name: 'style', type: 'StairStyle', description: 'straight | l-shape | u-shape | spiral | curved | winder' },
      { name: 'width', type: 'number', description: 'Clear tread width in metres (0.6m – 3.0m).' },
      { name: 'height', type: 'number', description: 'Total vertical rise from floor to floor (m).' },
      { name: 'length', type: 'number', description: 'Total horizontal going / run length (m).' },
      { name: 'structure', type: 'StairStructure', description: 'closed | open | floating | mono-stringer' },
      { name: 'railing', type: 'RailingConfig', description: 'none | left | right | both' },
      { name: 'idealStepHeight', type: 'number', description: 'Target riser height (standard: 0.175m).' },
      { name: 'handrailHeight', type: 'number', description: 'Vertical handrail height above tread nosing (standard: 0.90m–1.0m).' },
      { name: 'isParametric', type: 'boolean', description: 'Whether risers auto-calculate to satisfy building code ergonomics.' },
    ],
  },
  {
    name: 'LandscapePlant',
    path: 'Model.plants[] / sdk.landscape',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier.' },
      { name: 'species', type: 'PlantSpeciesId', description: 'english_oak | mediterranean_cypress | scots_pine | japanese_maple | boxwood_shrub | hydrangea_bush | ribbon_grass' },
      { name: 'position', type: '[number,number,number]', description: 'Ground placement coordinates.' },
      { name: 'scale', type: 'number', description: 'Foliage scale multiplier (0.5 – 2.5).' },
      { name: 'rotation', type: 'number', description: 'Plan-view Y-axis rotation in radians.' },
    ],
  },
  {
    name: 'TimberFramingMember',
    path: 'Model.timberFraming[] / sdk.architecture',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier.' },
      { name: 'memberType', type: 'FramingType', description: 'stud | sole-plate | top-plate | rafter | ridge-beam | collar-tie' },
      { name: 'species', type: 'TimberSpecies', description: 'douglas-fir | pine | oak | cedar' },
      { name: 'dimensions', type: '[number,number,number]', description: 'Width, depth, and length of the timber member in metres.' },
      { name: 'position', type: '[number,number,number]', description: 'Center position in 3D space.' },
      { name: 'color', type: 'string', description: 'Natural timber stain hex colour.' },
    ],
  },
  {
    name: 'DimensionAnnotation',
    path: 'Model.dimensions[] / sdk.measurement',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier.' },
      { name: 'start', type: '[number,number,number]', description: 'Start anchor point in world coordinates.' },
      { name: 'end', type: '[number,number,number]', description: 'End anchor point in world coordinates.' },
      { name: 'label', type: 'string', description: 'Measurement distance text with unit suffix.' },
      { name: 'unit', type: 'MeasurementUnit', description: 'm | ft | in | mm' },
    ],
  },
  {
    name: 'CustomToolbarModel',
    path: 'Model.customToolbars[] / sdk.toolbars',
    fields: [
      { name: 'id', type: 'string', description: 'Unique identifier for the toolbar instance.' },
      { name: 'title', type: 'string', description: 'Toolbar display title and drag banner label.' },
      { name: 'position', type: 'ToolbarPosition', description: 'top-left | top-center | top-right | bottom-left | bottom-center | bottom-right | floating | dock-left | dock-top | dock-bottom' },
      { name: 'orientation', type: 'ToolbarOrientation', description: 'horizontal | vertical' },
      { name: 'floatPosition', type: '{ x: number; y: number }?', description: 'Viewport pixel offset when floating.' },
      { name: 'items', type: 'CustomToolbarItem[]', description: 'List of extension action buttons.' },
      { name: 'collapsed', type: 'boolean', description: 'Whether the toolbar body is currently minimized.' },
      { name: 'closable', type: 'boolean', description: 'Whether the close button is visible.' },
    ],
  },
  {
    name: 'DeveloperSDKFacade',
    path: 'window.sdk (Runtime Global)',
    fields: [
      { name: 'toolbars', type: 'ToolbarsSubsystem', description: 'create, addButton, addToBasicToolbar, configureToolbar, configureButton, configureBasicToolbarButton, getToolbar, getButton, removeButton, removeFromBasicToolbar, removeToolbar, list, getBasicToolbarButtons, clear' },
      { name: 'architecture', type: 'ArchitectureSubsystem', description: 'createRoof, configureRoofDefaults, createStairs, configureStairDefaults, createRoom, configureRoomDefaults, createWall, configureWallDefaults, generateTimberFraming, configureTimberFramingDefaults' },
      { name: 'landscape', type: 'LandscapeSubsystem', description: 'addPlant, addSiteFurniture, configureSculptSettings, getSculptSettings, configureRoadSettings, getRoadSettings, configureLandscapeDefaults' },
      { name: 'materials', type: 'MaterialsSubsystem', description: 'applyMaterial, setEdgeLines, configureMaterialDefaults, getMaterialDefaults, listPresets' },
      { name: 'measurement', type: 'MeasurementSubsystem', description: 'addDimension, measureDistance, setUnit, getUnit, configureMeasurementSettings, getMeasurementSettings' },
      { name: 'selection', type: 'SelectionSubsystem', description: 'select, deselectAll, getSelected, group, ungroup, duplicateObject, alignObjects, isolateObject, transformObject' },
      { name: 'camera', type: 'CameraSubsystem', description: 'setProjection, resetView, setDepthClipping, setAutoOrbit, focusObject' },
      { name: 'ai', type: 'AISubsystem', description: 'generateModel, openRenderer, askAssistant' },
      { name: 'worldView', type: 'WorldViewSubsystem', description: 'importMap, setLocation, setRadius, setZoom' },
      { name: 'scene', type: 'SceneSubsystem', description: 'getStats, exportJSON, importJSON, clearScene, undo, redo' },
      { name: 'diagLog', type: '(cat, msg, val) => void', description: 'Records telemetry entries to diagnostic logging feed.' },
    ],
  },
];
